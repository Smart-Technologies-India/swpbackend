...
export class MarriageRegisterService {
  ...
  async getAllMarriageRegister() {
    const marriageRegister = await this.prisma.marriage_register.findMany({
  ...
  async getMarriageRegisterById(id: number) {
    const marriageRegister = await this.prisma.marriage_register.findFirst({
      where: { id, deletedAt: null },
    });
    if (!marriageRegister)
      throw new BadRequestException('No Marriage Register exist with this id.');
    return marriageRegister;
  }

  async createMarriageRegister(marriageRegister: CreateMarriageRegisterInput) {
    ...
    const MarriageRegister = await this.prisma.marriage_register.create({
      data: dataToCreate,
    });

    if (!MarriageRegister)
      throw new BadRequestException('Unable to create Marriage Register');
    return MarriageRegister;
  }
  ...
  async updateMarriageRegisterById(
    ...
    const updatedmarriageRegister = this.prisma.marriage_register.update({
      where: { id: marriageRegister.id },
      data: dataToUpdate,
    });
    if (!updatedmarriageRegister)
      throw new BadRequestException('Unable to update Marriage Register.');
  ...
  async deleteMarriageRegisterById(
    ...
    const deleteMarriageRegister = this.prisma.marriage_register.update({
      where: { id: marriageRegister.id },
