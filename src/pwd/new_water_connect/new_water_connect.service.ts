...
export class NewWaterConnectService {
  ...
  async getNewWaterConnectById(id: number) {
    const newWaterConnect =
      await this.prisma.new_water_connection.findFirst({
  ...
  async updateNewWaterConnectById(
    newWaterConnect: UpdateNewWaterConnectInput,
  ) {
    const dataToUpdate: {
      [key: string]: any;
    } = {};
  ...
  async deleteNewWaterConnectById(
    newWaterConnect: UpdateNewWaterConnectInput,
  ) {
